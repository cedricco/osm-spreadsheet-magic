...
def main():
    ...
    print("Columns OK")
    with open('utility_files/template_umap.umap') as umap_template_file:
        umap_data = json.load(umap_template_file)
...
%s
                    ...
                    umap_data["layers"][layer_number]["features"].append({
                        "type": "Feature",
                        "properties": {
                            "description": text,
                            "name": emoji + " " + row[3],
    ...
    with open('output.umap', 'w') as outfile:
        json.dump(umap_data, outfile)
